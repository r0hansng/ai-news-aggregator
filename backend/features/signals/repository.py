...
This module provides the Repository pattern for managing 'Signals'.
It handles deduplicated ingestion and specialized queries for the
normalization and curation pipelines.
"""
...
from .model import AnthropicArticle, OpenAIArticle, YouTubeVideo


class SignalRepository:
    """
    Data access layer for ingestion signals (YouTube, OpenAI, Anthropic).

    This repository handles the high-volume ingestion of raw news items.
    It is optimized for 'Deduplicated Bulk Creation' to ensure that we don't
    process the same news item multiple times across different scrapes.
    """

    def __init__(self, session: Optional[Session] = None):
        """
        Initializes the repository with a session.
        Defaults to the global get_session() if none provided.
        """
    ...
    def get_articles_without_digest(self, limit: Optional[int] = None) -> List[Dict[str, Any]]:
        ...
        youtube_videos = (
            ...
            .filter(
                YouTubeVideo.transcript.isnot(None), YouTubeVideo.transcript != "__UNAVAILABLE__"
            )
            .all()
        )
        ...
        anthropic_articles = (
            self.session.query(AnthropicArticle).filter(AnthropicArticle.markdown.isnot(None)).all()
        )
